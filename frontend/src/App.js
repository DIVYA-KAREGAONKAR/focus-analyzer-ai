...
const adviceRes = await getAdvice(status, Math.round(payloadForAI.active_ratio * 100), switchCount);
  ...
  const downloadPDF = () => {
    ...
    // 1. Calculate Score (Standardized)
    ...
    doc.text("Focus Analyzer Pro", 10, 20);
    
    doc.setFontSize(10);
    doc.setTextColor(100);
    doc.text("Performance Report", 10, 26);
    ...
    doc.setFontSize(14);
    doc.setTextColor(55);
    doc.text(`Session Status: ${status}`, 20, 65);

    doc.setFontSize(16);
    doc.setTextColor(0);
    // Added the "%" sign explicitly here
    doc.text(`Focus Intensity: ${focusScore}%`, 20, 73); 

    // --- AI ANALYSIS SECTION ---
    doc.setFontSize(14);
    doc.setTextColor(59, 130, 246);
    doc.text("🚀 Flow Analysis", 10, 100);

    doc.setFontSize(11);
    doc.setTextColor(50);
    
    // Clean up the advice text (remove ** bold markers if they exist)
    const cleanAdvice = (advice || "Analyzing...").replace(/\*\*/g, '');
    
    // Split text to fit page width
    const splitAdvice = doc.splitTextToSize(cleanAdvice, 180);
    doc.text(splitAdvice, 10, 110);

    // --- FOOTER ---
    doc.setFontSize(9);
    doc.setTextColor(150);
    doc.text("Generated by Focus Analyzer AI", 10, 280);
